import { GetObjectRequestDto } from '../dto';
import { ISpatialID, ISpatialInfoResponse } from '../interfaces';
import { faker } from '@faker-js/faker';

export function getSpatialInfo(objectId: string): ISpatialInfoResponse {
  return {
    result: {
      objectId: objectId,
      terrain: {
      ...
      channel: {
        ...
        channelValues: [{ id: { ID: faker.datatype.uuid() } }],
        junctionValues: [
          {
            id: { ID: faker.database.mongodbObjectId() },
            connectedChannelIds: [faker.datatype.uuid()],
          },
        ],
      },
      overlayArea: {
